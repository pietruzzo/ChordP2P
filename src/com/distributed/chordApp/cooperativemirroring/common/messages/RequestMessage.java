...
public class RequestMessage implements Serializable {
    ...
    /**
     * Method used for writing a concise representation of the request
     * @return
     */
    public String conciseToString(){
        String state = "REQUEST\n{" ;

        state += "[sender=@" + this.getOriginalSenderIP() + ":" + this.getOriginalSenderPort() + "] ";
        state += "[operation=";
        if(this.getDepositResource()){
            state +="deposit] ";
        }else{
            state += "retrieve] ";
        }
        state +="[resource ID=" + this.getResourceID() + "]}";
